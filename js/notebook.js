...
function crearCard(producto) {
    return `<div class="productos text-center pb-2 card rounded colorCard m-3" style="width: 16rem;">
                ...
                <div class="card-body text-center">
                    ...
                    <button id=${producto.id} class=" btn btn-dark container btn-agregar-notebook">Agregar</button>
                </div>
            </div>`;
}
...
function cargarProductosNotebook() {
    fetch(URL)
        .then((response) => response.json())
        .then((productos) => {
            const productosNotebook = productos.filter(producto => producto.categoria === 'notebook');
            cargarProductos(productosNotebook);
        })
...
function cargarProductos(arrayProductos) {
    ...
    agregarClickEnBotones("button.btn-agregar-notebook");
}

// Cargar productos de la categoría "Celulares" al cargar la página
cargarProductosNotebook();

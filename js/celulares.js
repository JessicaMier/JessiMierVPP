...
function crearCard(producto) {
    return `<div class="productos text-center pb-2 card rounded colorCard m-3" style="width: 16rem;">
                ...
                <div class="card-body text-center">
                    ...
                    <button id=${producto.id} class=" btn btn-dark container btn-agregar-celulares">Agregar</button>
                </div>
            </div>`;
}
...
function cargarProductosCelulares() {
    fetch(URL)
        .then((response) => response.json())
        .then((productos) => {
            const productosCelulares = productos.filter(producto => producto.categoria === 'celulares');
            cargarProductos(productosCelulares);
        })
...
function cargarProductos(arrayProductos) {
    ...
    agregarClickEnBotones("button.btn-agregar-celulares");
}
